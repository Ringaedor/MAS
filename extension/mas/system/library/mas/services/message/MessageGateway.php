...
class MessageGateway
{
    ...
    protected function executeWithRetry(string $providerCode, string $type, array $payload, string $messageId): array
    {
        ...
        while ($attempt < $this->maxRetries) {
            ...
            $this->updateQueuedMessage($row['id'], 'error', ['error' => $lastException->getMessage()]);
        }
     ...
     * @return void
     */
    protected function updateQueuedMessage(int $id, string $status, array $result): void
    {
        $allowed = ['sent', 'error', 'processing'];   // nuova whitelist
