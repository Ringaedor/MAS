...
class PaymentGateway
{
    ...
    /**
     * Updates performance metrics and persists to database.
     *
     * @param string $code
     * @param string $action
     * @param float $time
    ...
    protected function logPerformance(string $code, string $action, float $time, bool $success): void
    {
        ...
        // ✅ NUOVO: Persist to database table mas_provider_metrics
        $this->persistMetric($code, $action, $time, $success);
    }
    
    /**
     * Persists single metric entry to database.
     *
     * @param string $provider
     * @param string $action
     * @param float $responseMs
     * @param bool $success
     * @return void
     */
    protected function persistMetric(string $provider, string $action, float $responseMs, bool $success): void
    {
        try {
            $this->db->query("
            INSERT INTO `mas_provider_metrics` SET
            `provider`     = '" . $this->db->escape($provider) . "',
            `category`     = 'payment',
            `action`       = '" . $this->db->escape($action) . "',
            `success`      = '" . (int)$success . "',
            `response_ms`  = '" . (int)round($responseMs * 1000) . "',
            `created_at`   = NOW()
        ");
        } catch (\Exception $e) {
            // Log error but don't break payment flow
            $this->log->write('PaymentGateway: Failed to persist metric - ' . $e->getMessage());
        }
    }
